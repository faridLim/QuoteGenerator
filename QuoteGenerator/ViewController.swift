...
class ViewController: UIViewController {
    ...
    @IBAction func tabQuoteBtn(_ sender: Any) {
        let random = Int(arc4random_uniform(5)) //0~5사이에 난수 값 생성
        let  quote = self.quotes[random]
        self.quoteLabel.text = quote.contents  //label에 텍스트를 입력
        self.nameLabel.text = quote.name //label에 텍스트를 입력
